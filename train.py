...
import open_clip
from dataset import VisaDataset, MVTecDataset, Crack500Dataset
...
def train(args):
    ...
    if args.dataset == 'mvtec':
        ...
        train_data = Crack500Dataset(root=args.train_data_path, transform=preprocess, target_transform=transform, aug_rate=args.aug_rate)
        ...
        for items in train_dataloader:
            ...
            with torch.cuda.amp.autocast():
                with torch.no_grad():
                    ...
                    #print(np.array(patch_tokens).cpu().shape)
                    text_features = []
                    for cls in cls_name:
...
if __name__ == '__main__':
    ...
    # path
    parser.add_argument("--train_data_path", type=str, default="./data/crack500_patch", help="train dataset path")
    parser.add_argument("--save_path", type=str, default='./exps/crack500_patch_cam_thr/vit_large_14_518', help='path to save results')
    parser.add_argument("--config_path", type=str, default='./open_clip/model_configs/ViT-L-14-336.json', help="model configs")
    # model
    parser.add_argument("--dataset", type=str, default='crack', help="train dataset name")
