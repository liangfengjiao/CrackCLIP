...
class CrackDataset(data.Dataset):
	def __init__(self, root, transform, target_transform, aug_rate, parse='train', mode='test', k_shot=0, save_dir=None, obj_name=None):
		self.root = root
		self.transform = transform
		self.target_transform = target_transform
		self.aug_rate = aug_rate
	...
	def combine_img(self, cls_name):
		img_paths = os.path.join(self.root, cls_name, self.parse)
		img_ls = []
		mask_ls = []
		for i in range(4):
			defect = os.listdir(img_paths)
			random_defect = random.choice(defect)
			files = os.listdir(os.path.join(img_paths, random_defect, 'image'))
			random_file = random.choice(files)
			img_path = os.path.join(img_paths, random_defect, 'image', random_file)
			mask_path = os.path.join(img_paths, random_defect, 'mask', random_file[:-4] + '.png')
			img = Image.open(img_path)
			img_ls.append(img)
			# img_mask1 = np.array(Image.open(mask_path))
			img_mask = np.array(Image.open(mask_path).convert('L')) > 0
			img_mask = Image.fromarray(img_mask.astype(np.uint8) * 255, mode='L')
			mask_ls.append(img_mask)
